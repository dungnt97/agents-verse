...
/* ---- S9 · Final CTA + footer ---- */
function FinalCTA({ onEnter, onRequestDemo, onContact }) {
  return (
...
function Footer({ onEnter, onNav }) {
  const go = (key) => { if (key === '_ws') onEnter && onEnter(); else onNav && onNav(key); };
  const cols = [
    { h:t('foot.product'), items:[[t('foot.workspace'),'_ws'],[t('foot.floor'),'_ws'],[t('foot.command'),'_ws'],[t('nav.pricing'),'landing']] },
    { h:t('foot.company'), items:[[t('foot.about'),'about'],[t('trust.eyebrow'),'landing'],[t('foot.careers'),'careers'],[t('foot.contact'),'contact']] },
    { h:t('foot.resources'), items:[[t('foot.cases'),'cases'],[t('nav.how'),'landing'],[t('foot.guarantees'),'guarantees'],[t('foot.status'),'status']] },
  ];
  return (
      ...
      <div style={{ ...wrap, padding: '56px 28px 40px' }}>
        ...
        <div className="row between" style={{ marginTop: 48, paddingTop: 24, borderTop: '1px solid var(--border)', flexWrap: 'wrap', gap: 12 }}>
          ...
          <span className="row" style={{ gap: 18, fontSize: 13, color: 'var(--ink-3)' }}>
            <a href="#" onClick={e=>{e.preventDefault(); go('privacy');}} style={{cursor:'pointer'}}>{t('foot.privacy')}</a><a href="#" onClick={e=>{e.preventDefault(); go('terms');}} style={{cursor:'pointer'}}>{t('foot.terms')}</a><a href="#" onClick={e=>{e.preventDefault(); go('security');}} style={{cursor:'pointer'}}>{t('foot.security')}</a>
          </span>
        </div>
      </div>
    </footer>
...
Object.assign(window, { Pricing, TrustSafety, FinalCTA, Footer });
