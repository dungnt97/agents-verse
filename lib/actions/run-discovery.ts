...
import { activity, leads } from '@/lib/db/schema';
...
export async function runDiscovery(input: { industry?: string; city?: string }): Promise<DiscoveryResult> {
  if (!USE_DB) {
    return { found: 0, enriched: 0, upserted: 0, message: 'Discovery requires the database (set USE_DB=true).' };
  }
  if (!process.env.GOOGLE_MAPS_API_KEY) {
    return { found: 0, enriched: 0, upserted: 0, message: 'GOOGLE_MAPS_API_KEY is not configured.' };
  }
  ...
  if (cap > 0) {
    ...
    const [{ value: todayCount }] = await db
      .select({ value: count() })
      .from(leads)
      .where(and(isNotNull(leads.placeId), gte(leads.createdAt, since)));
    if (todayCount >= cap) {
      return { found: 0, enriched: 0, upserted: 0, message: `Daily discovery cap (${cap}) reached.` };
    }
  }
  ...
  return {
    ...
    upserted,
    message: `Found ${places.length}, enriched ${rows.length}, saved ${upserted} in ${city}.`,
  };
}
