...
function buildCipherPrompt({ company, industry, city, summary }: CipherInput): string {
  return [
    ...
    `tune ogTitle/ogDescription for a share card (punchier, benefit-first). Write EVERY value in natural`,
    `Vietnamese (the customer's language) — the business and its customers are in ${city}, Vietnam.`,
    ``,
