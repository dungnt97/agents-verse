...
// Pass 1 — write the tight, concrete creative-director spec the build pass will follow.
export const atlasDirector: AgentDef<DirectorInput, string> = {
  ...
  buildPrompt: ({ input, dna }) => buildDirectorPrompt(input, dna),
  validate: (raw) => raw.trim(),
};

// Pass 4 — consolidate the expert review board's notes into one de-duplicated, prioritized fix list.
