...
import { reapStaleRuns } from './functions/reap-stale-runs';
...
async function main(): Promise<void> {
  const connection = await connect({
    apps: [
      {
        client: inngest,
        functions: [runAudit, runDemoGen, orchestratePipeline, handleReply, runOutreach, runBuild, runSupport, sendProposal, autoDiscovery, reapStaleRuns],
      },
    ],
  });
  console.log(
    '[worker] connected to Inngest; audit + demo + orchestrator + closer + outreach + build + support + proposal + auto-discovery + reap-stale-runs functions registered',
  );
