...
- **Greenfield branch** — `if (!/^https?:\/\//i.test(lead.url ?? ''))`: `save-greenfield` (audit row + lead write-back + `audit_jobs` = `done`) → `emit-audit-completed` (if `runId`) → `log-activity-greenfield` → return. **No PSI, no Playwright, no Gemini — the only fully keyless audit path.**
- **Normal branch** — `pagespeed` (`runPerformanceAudit`) → `screenshot-and-score` (capture, cache the base64 desktop PNG into `audit_screenshots`, and call `scoreScreenshots` — all in ONE step) → `save` (`mapAuditResult` → upsert `audits` + lead write-back + `audit_jobs` = `done`) → `emit-audit-completed` (if `runId`) → `log-activity`.
...
The gate is the scheme test in `run-audit.ts`; the placeholder `'(no site yet)'` is written by `lib/discovery/map-place-to-lead.ts` (`url: enrichment.websiteUri ?? '(no site yet)'`). Change the brief in `greenfield-audit.ts` (`scores` must stay a full `ScoreProfile`; `sections` becomes the demo's section order) and remember the write-back.

## Traps

- **The greenfield gate is a URL-*scheme* test, not a "has a website" test.** A lead whose `url` is a bare domain (`atlasdentalhou.com` — exactly how the mock/seeded leads in `lib/data/index.ts` are stored, and what `SEED_DEMO_DATA=true` puts in Postgres) is silently audited as greenfield: all-zero scores, `site` 0, `score` 40 — even though it has a real site. Both engines and the screenshotter normalise a bare host to `https://`; the gate is the only place that does not. **Leads must store an `http(s)://`-prefixed `url`.** Nothing enforces this and no test covers the branch.
- **The performance path bypasses the SSRF guard.** `assertSafeUrl` is **module-private inside `screenshot.ts`** and is reached only via `captureScreenshots` — which runs *after* `runPerformanceAudit`. Under `AUDIT_PROVIDER=lighthouse` the worker's own Chromium therefore navigates the raw `lead.url` from *inside* the compose network (`db`, `redis`, `inngest`, `9router` are all reachable). New code must use the exported `assertPublicUrl` / `safeFetch` from `lib/discovery/safe-fetch.ts`.
...
- **The audit does NOT degrade without `GEMINI_API_KEY`** — for any lead *with* a website the vision step throws, the error escapes to the function catch, `audit_jobs.status` becomes `failed`, and (for an orchestrated run) `onFailure` fails the whole pipeline run. Only the greenfield path survives keyless.
