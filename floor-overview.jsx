...
function MetricStat({ label, value, decimals=0, prefix='', suffix='', icon, spark, sparkColor, meter, meterPct, delta, deltaUp=true, accent }) {
  return (
    <div className="card" style={{ padding:'15px 17px', minWidth:0 }}>
      ...
      <div className="row between" style={{ alignItems:'flex-end' }}>
        ...
        {meter && <div style={{ width:78 }}>
          <div className="track" style={{ height:7 }}><i style={{ width:meterPct+'%', background: meterPct>80?'var(--warning)':'var(--primary)' }} /></div>
...
function ActivityRow({ a }) {
  const statusColor = { success:'var(--success)', warning:'var(--warning)', review:'var(--warning)', info:'var(--info)', danger:'var(--danger)' }[a.status] || 'var(--ink-3)';
  return (
...
function EscalationMini({ e, onAction }) {
  const sevCls = e.sev==='high'?'badge-danger':e.sev==='medium'?'badge-warning':'badge-neutral';
  const kindIcon = { human:'user', deal:'deals', cost:'dollar' }[e.kind] || 'alert';
  return (
...
function RoomPeek({ roomId, onClose, onAction, goRoom }) {
...
function FloorOverview({ onAction, goCommand, goRoom }) {
  const [peek, setPeek] = useState(null);
  const m = AV.metrics;
  const now = new Date();
  const hr = now.getHours();
  const greet = hr<12?t('ov.greet1'):hr<18?t('ov.greet2'):t('ov.greet3');
  return (
      ...
      <div style={{ display:'grid', gridTemplateColumns:'minmax(0,1fr) 360px', gap:20, alignItems:'start' }} className="floor-grid">
        ...
        <div className="card" style={{ padding:0, overflow:'hidden' }}>
          ...
          <div style={{ padding:18 }}>
            <FloorMap selected={peek} onRoom={setPeek} height={540} />
          </div>
        </div>
...
Object.assign(window, { FloorOverview, MetricStat, ActivityRow, EscalationMini, RoomPeek });
