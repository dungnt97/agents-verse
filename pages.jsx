/* =========================================================================
   AGENTS VERSE — Public info pages (footer links)
   About · Careers · Contact · Case studies · Guarantees · Status · Legal
   ========================================================================= */

function InfoNav({ theme, onToggle, onEnter, onRequestDemo, onHome }) {
  return (
...
function InfoHero({ eyebrow, title, sub, max=720 }) {
  return (
    <div style={{ ...wrap, paddingTop:80, paddingBottom:24 }}>
      ...
      {sub && <Reveal delay={120}><p style={{ fontSize:19, color:'var(--ink-2)', lineHeight:1.5, marginTop:20, maxWidth:600, textWrap:'pretty' }}>{sub}</p></Reveal>}
    </div>
  );
}

function InfoCTA({ onRequestDemo }) {
  return (
...
/* ---- ABOUT ---- */
function AboutPage({ onRequestDemo }) {
  const stats = [['148','sites scanned daily'],['3.4×','higher reply rate'],['48h','lead to live demo'],['8','AI departments']];
  const values = [
      ...
      <div style={{ ...wrap, paddingTop:40, paddingBottom:20 }}>
        <div style={{ display:'grid', gridTemplateColumns:'minmax(0,1fr) minmax(0,1.2fr)', gap:56 }} className="why-grid">
          ...
          <div className="col" style={{ gap:18 }}>
            <Reveal delay={80}><p style={{ fontSize:16.5, color:'var(--ink-2)', lineHeight:1.6, textWrap:'pretty' }}>We started Agents Verse with a simple frustration: small businesses are quoted thousands and shown nothing until weeks later. Meanwhile their customers are leaving sites that look a decade old.</p></Reveal>
            <Reveal delay={140}><p style={{ fontSize:16.5, color:'var(--ink-2)', lineHeight:1.6, textWrap:'pretty' }}>So we built a virtual company that does the opposite. Research agents find the businesses. Audit agents score the current site. Design and code agents build a real, working redesign. Sales agents prepare a personalized message. Only then — and only when it matters — does a human step in.</p></Reveal>
            <Reveal delay={200}><p style={{ fontSize:16.5, color:'var(--ink-2)', lineHeight:1.6, textWrap:'pretty' }}>The result is an agency that moves at software speed but earns trust like a craftsperson: by showing the work.</p></Reveal>
          </div>
        </div>
      </div>
...
/* ---- CAREERS ---- */
function CareersPage({ onRequestDemo }) {
  const roles = [
...
/* ---- CONTACT ---- */
function ContactForm({ onRequestDemo }) {
...
function ContactPage({ onRequestDemo }) {
  const methods = [
...
/* ---- CASE STUDIES ---- */
function CasesPage({ onRequestDemo }) {
  const cases = [
      ...
      <div style={{ ...wrap, paddingTop:20 }}>
        <div className="col" style={{ gap:24 }}>
          {cases.map((c,i)=>(<Reveal key={i} delay={i*80}>
            <div className="card-elev" style={{ padding:20, display:'grid', gridTemplateColumns:'minmax(0,1.3fr) minmax(0,1fr)', gap:26, alignItems:'center', boxShadow:'var(--sh-md)' }} className="showcase-grid">
              ...
              <div>
                ...
                <p style={{ fontSize:15, color:'var(--ink-2)', lineHeight:1.55, fontStyle:'italic', textWrap:'pretty' }}>“{c.quote}”</p>
              </div>
            </div>
          </Reveal>))}
        </div>
...
/* ---- GUARANTEES ---- */
function GuaranteesPage({ onRequestDemo }) {
  const gs = [
...
/* ---- STATUS ---- */
function StatusPage({ onRequestDemo }) {
  const comps = [
...
/* ---- LEGAL ---- */
function LegalDoc({ title, updated, intro, sections }) {
  return (
    <>
        ...
        <div style={{ maxWidth:760 }}>
          ...
          {sections.map((s,i)=>(<Reveal key={i} delay={Math.min(i*40,200)}>
            <div style={{ marginBottom:28 }}>
              ...
              <p style={{ fontSize:15.5, color:'var(--ink-2)', lineHeight:1.65, textWrap:'pretty' }}>{s.body}</p>
            </div>
          </Reveal>))}
...
function InfoPage({ page, theme, onToggle, onEnter, onRequestDemo, onNav }) {
  let body;
  ...
  else if (LEGAL[page]) body = <LegalDoc {...LEGAL[page]} />;
  return (
    <div style={{ background:'var(--bg)', minHeight:'100vh' }}>
      <InfoNav theme={theme} onToggle={onToggle} onEnter={onEnter} onRequestDemo={onRequestDemo} onHome={()=>onNav('landing')} />
      <main>{body}</main>
...
Object.assign(window, { InfoPage, InfoNav, INFO_PAGES: ['about','careers','contact','cases','guarantees','status','privacy','terms','security'] });
